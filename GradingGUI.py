...
menu_def = [['Toiminto',['Export opiskelijadata', 'Laske virhepisteet', 'Päivitä virhepisteet']]]
...
treecol = [[sg.Tree(data=studentdata,
                   ...
                   sg.Multiline(key = 'virheteksti', default_text = 'Laita tähän virhekoodia.', horizontal_scroll = True, autoscroll = True, size = (25,10)), 
                   sg.Multiline(key = "-ERROROUT-",expand_x = True, autoscroll = True, horizontal_scroll = True, size = (25,10), tooltip = 'Syötetty koodi näkyy tässä laatikossa.')
                   ],
        ...
        sg.Button('Kirjoita arvostellut työt tiedostoon', key='WRITE'), sg.Button("Kopioi teksti leikepöydälle", key = "-COPY-"),sg.Button('Exit')]]
...
def main():
    ...
    baseinfo, category_list = guilib.initiate_problem_list(treedata) # WIP: Tee opiskelijakohtainen kopio category_list:istä 
...
###Adding checkboxes next to student names for easier marking ###
    ...
    #####################Counting mistake points ##############################
            ...
            try:
                ...
                category_sum = guilib.clear_sums(window, baseinfo, treedata)
                selected_student = students[path2]
                guilib.remove_excludes(selected_student, baseinfo) # Remove excluded error from list if found
                    ...
                    if selected_student != []:
                        if error.error in selected_student.keys():
                            ...
                            if parent_node.parent == '':
                                ...
                                category_sum = category_sum + abs(selected_student[error.error])
                                window['-TREE-'].update(key = kategoria, value = category_sum)
                            errorpoints, alternative_added = guilib.count_alternative_points(error, baseinfo, selected_student, alternative_added, errorpoints)
